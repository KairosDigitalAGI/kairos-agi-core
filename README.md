...
**Integration Control Plane:** YouTube e Instagram compartilham OAuth server-side, `state` assinado, identidade consultada no provedor e o cofre cifrado `command.integracoes_tokens`. As URLs sociais foram consolidadas em uma função dinâmica para manter a Vercel Hobby gratuita. O canal real [Kairos Digital](https://www.youtube.com/channel/UC2TqvTgMsTkywGQS3oiYDsg) (`@KairosDigitalAGI`) está conectado com `youtube.readonly` e `youtube.upload`; uploads continuam privados por padrão e dependem de aprovação explícita no job. O app Meta foi criado com escopos de perfil, publicação, comentários e Direct; o vínculo da conta profissional ainda depende do segredo em produção e do consentimento OAuth. Valores secretos nunca chegam ao frontend e conexão não equivale a publicação.
