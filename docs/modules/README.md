# Módulos do Kairos Core v0.1

- [Founder Dashboard](FOUNDER_DASHBOARD.md)
- [World 0.1](WORLD_0_1.md)
- [Task System](TASK_SYSTEM.md)
- [CRM](CRM.md)
- [Instagram Engine](INSTAGRAM_ENGINE.md)
- [Instagram Engine — Missão 002](INSTAGRAM_ENGINE_V0_2.md)
- [Atendimento Instagram — comentários e Direct](INSTAGRAM_ENGAGEMENT.md)
- [Clone Engine — Missão 003](CLONE_ENGINE_V0_3.md)
- [Video Engine — Missão 004](VIDEO_ENGINE_V0_4.md)
- [Política de dados reais](REAL_DATA_POLICY.md)
- [Kairos Coins](KAIROS_COINS.md)
- [Módulos preparados](PLACEHOLDERS.md)

O estado operacional usa somente registros criados pelo Founder e dados derivados deles. Valores externos ausentes aparecem como indisponíveis. O planejamento editorial e o Clone guardam metadados locais; YouTube e Instagram usam OAuth e token cifrado no servidor. O atendimento Instagram aguarda ativação do webhook e migration. O Video Engine local processa arquivos no dispositivo.
