...
| Supabase | Em uso (leitura + escrita + Storage) | Free, projeto do kairos-command | Fase 1/2 da Missão 006 leem schema `command` via Service Role, server-side; Fase 3 grava `command.content_jobs` (ideias do Founder); Fase 4 grava `command.integracoes_tokens` (tokens OAuth cifrados); Fase 5 grava roteiro em `command.content_assets`; Fase 6 sobe imagem gerada no bucket Storage `content-assets` e grava `storage_path`. Migrations `0020_content_engine.sql`, `0021_integracoes_tokens.sql` e `0022_content_assets_bucket.sql` redigidas, pendentes de aplicar em produção pelo Founder. Nenhum projeto pausado, criado ou alterado |
