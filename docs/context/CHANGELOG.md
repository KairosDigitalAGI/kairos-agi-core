...
## Missão 006, Fase 18 — X (Twitter) em modo display/link, sem automação (23/09/2026, trabalho noturno autônomo)
- `computeContentPipeline()` (`api/_content.js`) passou a expor `videoUrl` por job — segunda consulta a `command.content_assets` (`tipo=eq.video`, `job_id=in.(...)`) reaproveitando o mesmo `storage_path` público que `postToYoutube`/`postToInstagram` já usam, nunca um novo dado inventado.
- `src/engines/video/distributionPackage.ts` (motor já existente desde a Missão 004, órfão de UI até hoje) ganhou `buildXShareText`/`validateXShareCaption`/`xShareComposeUrl` para o vídeo remoto real do Content Engine — reaproveita `validateXCaption`/`xComposeUrl` já testados, sem duplicar a validação de 280 caracteres. `distributionManifest` (o fluxo antigo de `StoredVideo` local em blob) permanece intocado, ainda sem consumidor de UI — dívida técnica registrada, não removido por não ter certeza de que está morto de vez.
- `ContentEnginePanel`: todo job com `videoUrl` ganha um campo de legenda + link "Abrir compositor do X", que monta `x.com/intent/post?text=...` com a legenda e o link do vídeo já publicado — abre o compositor oficial do X num novo separador, nunca chama a API do X nem publica sozinho. Aparece independente da etapa/publicação no YouTube/Instagram, já que o compartilhamento no X é manual e paralelo.
- 5 testes novos (`tests/content-engine.test.mjs`, `tests/video.test.mjs`), total 216/216 passando, typecheck e build limpos.
- **Nenhuma chamada à API do X ocorreu nem podia ocorrer** — o módulo só monta uma URL de compositor, mesmo padrão "display/link only" do backlog noturno.

## Missão 006, Fase 17 — webhook de DM/comentários do Instagram + rascunho de resposta por IA (23/09/2026, trabalho noturno autônomo)
- Migration `supabase/migrations/0026_instagram_webhook.sql` (mesmo desvio de convenção da 0025, mesmo motivo: kairos-command com mudanças alheias não commitadas): tabelas `command.integration_settings` (interruptor por provedor, genérico para reaproveitar em integrações futuras) e `command.instagram_webhook_events` (log append-only), RLS ligada, escrita só `service_role`.
- `api/integrations/instagram-webhook.mjs`: rota física NOVA e separada do catch-all `api/integrations/[...route].mjs` — único arquivo do Core no formato Web Standard `fetch(request)` em vez do handler Node `(req,res)`, porque a verificação `X-Hub-Signature-256` da Meta exige os bytes brutos do corpo, que os helpers automáticos da Vercel não preservam por arquivo (só via `NODEJS_HELPERS=0` do projeto inteiro, inviável). Custa 1 slot novo do teto de 12 (4/12 usados agora, 8 livres da Fase 15). Sem Basic Auth de propósito: quem chama é a Meta, a autenticidade vem da assinatura.
- `api/_instagram_webhook.js`: `verifyWebhookChallenge`/`verifyWebhookSignature` (handshake GET e HMAC-SHA256 do POST, ambos fail-closed sem as env vars), `parseMessagingEvents`/`parseCommentEvents` (mensagem confirmada no exemplo literal da doc do Messenger Platform; comentário reconstruído do esquema de campos documentado, sem exemplo literal publicado pela Meta — distinção registrada em comentário no código), `getAutoReplyEnabled`/`setAutoReplyEnabled` (interruptor OFF por padrão, nunca presume ligado sem a tabela), `generateSuggestedReply` (OpenRouter fixo, mesma convenção da Fase 16, nunca envia — só rascunha), `handleWebhookPayload` (orquestrador best-effort que nunca lança, para sempre responder 200 rápido à Meta mesmo com falha parcial de log).
- `api/integrations/[...route].mjs` ganhou as ações `auto-reply` GET/POST (Basic Auth, painel) para ligar/desligar o interruptor — únicas ações desta fase que continuam no router consolidado, já que não lidam com corpo bruto.
- UI: `src/core/useAutoReplyToggle.ts` (hook, mesmo contrato "indisponível nunca inventado" do resto do painel) + checkbox no card do Instagram em `IntegrationsPage.tsx`, OFF por padrão, com aviso quando a migration ainda não foi aplicada.
- 22 testes novos (`tests/instagram-webhook.test.mjs`): assinatura/handshake válidos e inválidos, parsing dos dois formatos de payload documentados, interruptor com/sem Supabase configurado, rascunho de resposta mockado (OpenRouter), orquestração completa com e sem auto-reply ligado, e a rota física `fetch(request)` fim a fim (GET challenge, POST assinatura inválida 401, POST assinado processado). Total 213/213 passando, typecheck e build limpos.
- **Nenhum envio real ocorreu esta noite** — nem mensagem, nem resposta a comentário, nem publicação: o módulo só grava um rascunho de texto no log, nunca chama nenhum endpoint de escrita da Meta. Proibição explícita do trabalho noturno respeitada.
- Pendências registradas em MORNING.md: `META_APP_SECRET`, `META_WEBHOOK_VERIFY_TOKEN` e a URL do webhook cadastrada no app da Meta (`https://<domínio-de-produção>/api/integrations/instagram-webhook`) dependem do Founder; migration 0026 pendente de aplicação e de cópia para kairos-command.

## Missão 006, Fase 16 — Estúdio Kairos, backend + UI sem gasto real (23/09/2026, trabalho noturno autônomo)
- Migration `supabase/migrations/0025_estudio_kairos.sql` (pousada no próprio kairos-agi-core, não em kairos-command — desvio de convenção registrado em MORNING.md porque o working tree de kairos-command tinha mudanças alheias não commitadas): tabelas `command.characters`, `command.reels`, `command.scenes`, `command.studio_spend` (ledger append-only) + bucket `"studio"`, mesma regra de ouro do schema (RLS ligada, escrita só `service_role`).
- `api/_studio.js`: `assertBudgetAvailable()` é o guard de orçamento — soma `studio_spend`, recusa (503) sem `STUDIO_BUDGET_USD` configurado e recusa (402) quando o gasto projetado estouraria o teto, SEMPRE antes de chamar qualquer provider pago. `generateCharacterPortrait()` é a única chamada paga hoje: passa pelo guard, gera via fal.ai, baixa do CDN da fal e sobe no bucket `"studio"` (nunca referencia a URL externa, que expira).
- `api/_providers/fal.js` ganhou `generateImage()` (modelo `fal-ai/flux/schnell`, ID conferido em 23/09/2026 na doc oficial fal.ai/models/fal-ai/flux/schnell) reaproveitando o mesmo polling assíncrono já testado de `generateVideo()` via um helper interno compartilhado — comportamento de `generateVideo()` inalterado (40 testes do Content Engine seguem verdes).
- `api/_studio_agents.js`: quatro agentes de texto (estrategista/roteirista/diretor/QA) com prompts versionados (`PROMPTS.<agente>.version`), rodando fixo no OpenRouter (não no `selectProvider()`/`auto` do resto do Core) — decisão deliberada porque o modelo padrão do OpenRouter neste repo já é `:free` de verdade, alinhado ao escopo desta fase de não gastar dinheiro.
- Recurso `studio` adicionado a `api/[resource]/[action].mjs` (mesmo router consolidado da Fase 15) com 12 ações (personagem/reel/cena + os quatro agentes) — nasceu direto consolidado, nunca foi arquivo próprio, mantendo 3/12 Serverless Functions.
- `EstudioKairosPanel` (dashboard): criar personagem, gerar retrato de referência, registrar reel com custo estimado exibido antes de aprovar, aprovar o gasto do reel. Edição de cena e disparo dos agentes de texto pela UI ficaram para uma próxima sessão — só o backend/rotas existem hoje (ver MORNING.md).
- 46 testes novos (`tests/studio.test.mjs`, `tests/studio-agents.test.mjs`, mais os acréscimos em `tests/resource-action-router.test.mjs`), zero chamada paga real — tudo mockado (fal.ai e OpenRouter). Total 191/191 passando, typecheck e build limpos.
- **Nenhuma geração paga rodou de verdade esta noite** — nem imagem (fal.ai/flux/schnell) nem texto (embora o OpenRouter padrão seja gratuito, nenhuma chamada de rede saiu do ambiente de teste): proibição explícita do trabalho noturno.
