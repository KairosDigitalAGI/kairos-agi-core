...
import { Camera, Clapperboard, Download, FileText, ImagePlus, LibraryBig, LockKeyhole, PanelsTopLeft, Pause, Play, Save, Sparkles, UsersRound } from 'lucide-react'
...
function downloadEpisodeDocument(episode: Episode) {
  const dialogue = (dialogueBeats[episode.number] ?? []).map(beat => `- **${beat.speaker}:** “${beat.line}”`).join('\n')
  const frames = (storyboards[episode.number] ?? []).map(frame => `### Cena ${frame.scene} · ${frame.title}\n${frame.duration} · Câmera: ${frame.camera} · Transição: ${frame.transition}\n${frame.visual}\n\nPrompt: ${frame.prompt}`).join('\n\n')
  const content = `# Kairos Signal — Episódio ${episode.number}: ${episode.title}\n\n## Visão\n${episode.summary}\n\n## Narração\n${episode.narration}\n\n## Roteiro completo\n${episode.screenplay}\n\n## Falas centrais\n${dialogue}\n\n## Storyboard e prompts\n${frames}\n\n## Direção de qualidade\nO universo dentro do computador é uma fantasia cinematográfica com luzes Kairos, vidro, ampulheta e avatares expressivos. As cenas do mundo real exigem realismo fotográfico contemporâneo: luz natural, textura de pele, poros, pupila, sombras físicas e profundidade de campo de cinema. A marca Kairos Digital entra pela composição local do lockup original, nunca como texto refeito pelo modelo.\n\n## Limites de produção\nEste documento é editorial. Gerar, enviar referências humanas, gastar crédito e publicar exigem os respectivos gates, confirmação de destino e retorno verificável do provider.`
  const url = URL.createObjectURL(new Blob([content], { type: 'text/markdown;charset=utf-8' }))
  const anchor = document.createElement('a')
  anchor.href = url; anchor.download = `kairos-signal-episodio-${episode.number}.md`; anchor.click(); URL.revokeObjectURL(url)
}
